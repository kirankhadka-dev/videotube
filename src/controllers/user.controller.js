...
const getUserChannelProfile = asyncHandler(async (req, res) => {
  try {
    ...
    const channel = await User.aggregate([
      ...
      {
        // Finding  the subsribers
        $lookup: {
          from: "subscriptions", //Model that is saved in the db
          localField: "_id",
    ...
    console.log("channel info after aggrregate ", channel);
    ...
    return res
      ...
      .json(
        new ApiResponse(
          200,
          channel[0],
          "user channel profile fetch successfully "
        )
      );
  } catch (error) {
    throw new ApiError(
      500,
      error?.message ||
        "something went  wrong while  retrieving channel profile"
    );
  }
});
